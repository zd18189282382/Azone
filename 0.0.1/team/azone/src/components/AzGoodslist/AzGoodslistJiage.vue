<template>
    <div @click="goto(shopa[2].id)">
      <ul>
        <li>
          <div class="proLeft">
            <img :src="shopa[2].picbig" alt=""/>
            <img :src="shopa[2].picjiao" alt=""/>
          </div>
          <div class="proRight">
            <p class="name">
              <i class="project-label">{{shopa[2].shopi}}</i>
              {{shopa[2].shopaont}}
            </p>
            <dl>
              <dd :key="index" v-for="(p,index) in shopa[2].suggest">{{p}}</dd>
            </dl>
            <p class="price">
              <span>¥</span>
              <span>{{shopa[2].price}}</span>
              <span>.00</span>
            </p>
            <p class="pingjia">
              <span>70万+条评价</span>
