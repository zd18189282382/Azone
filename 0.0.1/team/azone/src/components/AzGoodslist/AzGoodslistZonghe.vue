<template>
    <div @click="goto(shopa[0].id)">
      <ul>
        <li>
          <div class="proLeft">
            <img :src="shopa[0].picbig" alt=""/>
            <img :src="shopa[0].picjiao" alt=""/>
          </div>
          <div class="proRight">
            <p class="name">
              <i class="project-label">{{shopa[0].shopi}}</i>
             {{shopa[0].shopcont}}
            </p>
            <dl>
              <dd :key="index" v-for="(p,index) in shopa[0].suggest">{{p}}</dd>
            </dl>
            <p class="price">
              <span>¥</span>
              <span>{{shopa[0].price}}</span>
              <span>.00</span>
            </p>
            <p class="label">
              <span>抢</span>
              <span>{{shopa[0].gos}}</span>
            </p>
            <p class="pingjia">
              <span>70万+条评价</span>
              <span>好评率99%</span>
